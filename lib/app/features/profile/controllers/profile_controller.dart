...
class ProfileController extends GetxController {
  ...
  final String _cloudinaryUploadPreset = 'admin_profile'; // Use the unsigned preset for admin profile
  ...
  void onInit() {
    ...
    _cloudinary = CloudinaryPublic(_cloudinaryCloudName, _cloudinaryUploadPreset, cache: false);
    fetchProfileData();
  }
  ...
  Future<void> pickAndUploadImage() async {
    ...
    statusMessage.value = null;
    isError.value = false;
    String? oldImageUrl = profilePhotoUrl.value; // Store old URL before potential update
    ...
    try {
      ...
      if (pickedFile != null) {
        ...
        final String publicId = 'profiles/admin/$userId/profile_${DateTime.now().millisecondsSinceEpoch}';
        ...
        CloudinaryResponse response = await _cloudinary.uploadFile(
          CloudinaryFile.fromFile(
            ...
            folder: 'profiles/admin/$userId', // Optional: Organize in Cloudinary folders
        ...
        if (response.secureUrl.isNotEmpty) {
          ...
          await _profileDocRef!.update({
            ...
            'profilePhotoPublicId': response.publicId, // Store public ID for potential deletion
          ...
          if (oldImageUrl != null && oldImageUrl.isNotEmpty && oldImageUrl != newImageUrl) {
            ...
            final oldPublicId = _extractPublicIdFromUrl(oldImageUrl); // Helper needed
             if (oldPublicId != null) {
               print("TODO: Call backend to delete old Cloudinary image with public_id: $oldPublicId");
          ...
          _showStatus("Failed to upload image.", isError: true); // Keep the generic error for the user
        }
  ...
  String? _extractPublicIdFromUrl(String url) {
    try {
      ...
      if (pathSegments.length >= 6) { // Adjust index based on your URL structure
         ...
         if (uploadIndex != -1 && pathSegments.length > uploadIndex + 2) {
            ...
            String potentialPublicId = pathSegments.sublist(uploadIndex + 2).join('/');
            // Remove file extension
            if (potentialPublicId.contains('.')) {
               potentialPublicId = potentialPublicId.substring(0, potentialPublicId.lastIndexOf('.'));
            }
