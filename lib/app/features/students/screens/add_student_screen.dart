...
class _AddStudentScreenState extends State<AddStudentScreen> {
  ...
  Future<void> _fetchAvailableClasses() async {
    try {
      ...
      const defaultClasses = ['Grade 1', 'Grade 2', 'Grade 3', 'Grade 4', 'Grade 5', 'Grade 6', 'Grade 7', 'Grade 8', 'Grade 9', 'Grade 10', 'Grade 11', 'Grade 12'];
      final combined = {...classesFromDb, ...defaultClasses}.toList()..sort((a, b) {
  ...
  Future<void> _pickImage() async {
    ...
    try {
      final XFile? image = await picker.pickImage(source: ImageSource.gallery, imageQuality: 70); // Add quality constraint
  ...
  Future<String?> _uploadImageToCloudinary(XFile image) async {
    try {
      ...
      if (response.secureUrl.isNotEmpty) {
        ...
        throw Exception('Failed to upload image. Check Cloudinary logs or preset configuration.');
      }
    } catch (e) {
  ...
  Future<void> _selectDate(BuildContext context) async {
    final DateTime? picked = await showDatePicker(
      context: context,
      initialDate: _selectedDob ?? DateTime.now().subtract(const Duration(days: 365 * 10)), // Default ~10 years ago
  ...
  Future<void> _addStudent() async {
    ...
    if (!_formKey.currentState!.validate()) {
      ScaffoldMessenger.of(context).showSnackBar(
         const SnackBar(content: Text('Please fill all required fields.'), backgroundColor: kErrorColor),
       );
      return; // Don't proceed if form is invalid
    }
    if (_selectedImage == null) {
       ScaffoldMessenger.of(context).showSnackBar(
         const SnackBar(content: Text('Please upload a student photo.'), backgroundColor: kErrorColor),
       );
       return;
    }
  ...
  Future<void> _downloadQrCode() async {
     ...
     if (Platform.isAndroid) {
        ...
        if (androidInfo.version.sdkInt >= 33) { // Android 13+ requires photos permission
            status = await Permission.photos.request();
        } else { // Older Android versions use storage
            status = await Permission.storage.request();
        }
     } else { // iOS or other platforms typically use photos permission
         status = await Permission.photos.request();
     }


     if (status.isGranted) {
       setState(() => _isLoading = true); // Show loading indicator during capture/save
       try {
         // 2. Capture QR Code Widget as Image
         RenderRepaintBoundary boundary = _qrKey.currentContext!.findRenderObject() as RenderRepaintBoundary;
         // Ensure the boundary is ready
         await Future.delayed(const Duration(milliseconds: 100));
         ui.Image image = await boundary.toImage(pixelRatio: 3.0); // Increase pixelRatio for better quality
         ByteData? byteData = await image.toByteData(format: ui.ImageByteFormat.png);
         if (byteData == null) throw Exception("Could not capture QR code image.");
         Uint8List pngBytes = byteData.buffer.asUint8List();

         // 3. Save Image using saver_gallery
         final SaveResult result = await SaverGallery.saveImage(
             pngBytes, // Positional argument for image bytes
             fileName: "student_qr_${_qrData!.substring(0, 8)}", // Correct parameter name
             quality: 95, // High quality
             androidRelativePath: "Pictures/EduTrack", // Specify a subfolder in Pictures
         ...
         if (result.isSuccess) {
           ...
           throw Exception("Failed to save QR code: ${result.errorMessage ?? 'Save error'}");
         }
       } catch (e) {
      ...
      builder: (context, snapshot) {
        ...
        if (photoUrl != null && photoUrl.isNotEmpty) {
          profileWidget = CircleAvatar(
            ...
            onBackgroundImageError: (exception, stackTrace) { print("Error loading profile image: $exception"); },
          );
        }
        return Material(
            ...
            children: [
              ...
              _buildDropdownField(
               ...
               _buildTextField(
                ...
                label: 'Section',
                hint: 'Enter section (e.g., A, B)',
                icon: Icons.group_work_outlined,
                validator: (value) => value == null || value.isEmpty ? 'Please enter section' : null,
              ),
              _buildDateField(
                context: context,
                label: 'DOB',
                ...
                validator: (value) => _selectedDob == null ? 'Please select date of birth' : null,
              ),
              _buildDropdownField(
                value: _selectedSex,
                label: 'Sex',
              ...
              _buildTextField(
                ...
                icon: Icons.supervisor_account_outlined,
                 validator: (value) => value == null || value.isEmpty ? 'Please enter parent name' : null,
              ),
              _buildTextField(
              ...
              Text('Upload Photo', style: textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w600)),
      ...
      child: Column(
        ...
        children: [
          ...
          DropdownButtonFormField<String>(
            ...
            decoration: InputDecoration(
               prefixIcon: Icon(
                 label == 'Class' ? Icons.school_outlined : Icons.wc_outlined, // Icon based on label
                 size: 20
               ),
               // Using theme defaults for border, padding etc.
            ),
           ...
           TextFormField(
            ...
            decoration: InputDecoration(
              hintText: selectedDate == null
                  ? 'Select date'
                  : DateFormat('dd / MM / yyyy').format(selectedDate), // Format date
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  ...
                  style: BorderStyle.solid, // Dashed border is complex, using solid
                  width: 1.5,
                ),
              ),
              child: _selectedImage == null
                  ? Column(
                      ...
                      children: [
                        Icon(Icons.cloud_upload_outlined, size: 40, color: kLightTextColor),
                        const SizedBox(height: 8),
                        Text('Drop files here', style: kHintTextStyle),
                        Text('Supported format: PNG, JPG', style: kHintTextStyle.copyWith(fontSize: 11)),
                      ],
                    )
                  : ClipRRect( // Show selected image preview
                     borderRadius: BorderRadius.circular(kDefaultRadius * 0.8 - 1.5), // Adjust for border
          ...
          if (_selectedImage != null)
            Padding(
              ...
              child: Row(
                ...
                children: [
                  TextButton(
                    onPressed: () => setState(() => _selectedImage = null), // Clear selection
                  ...
                  ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      padding: const EdgeInsets.symmetric(horizontal: kDefaultPadding * 1.5, vertical: kDefaultPadding * 0.5),
                      minimumSize: Size.zero,
                    ),
                    child: const Text('Upload'), // Or 'Change' ? Let's keep 'Upload' for simplicity
                  ),
                ],
              ),
            ),
       ...
       child: Column(
         children: [
           ...
           ElevatedButton.icon(
             ...
             onPressed: _isLoading ? null : _downloadQrCode, // Disable while loading/saving QR
             style: ElevatedButton.styleFrom(
               backgroundColor: kPrimaryColor.withOpacity(0.9), // Use theme primary color
               padding: const EdgeInsets.symmetric(horizontal: kDefaultPadding * 1.5, vertical: kDefaultPadding * 0.7),
             ),
           ),
         ],
       ),
